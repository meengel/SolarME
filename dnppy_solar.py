### Jeff Ely (https://github.com/NASA-DEVELOP/dnppy/blob/master/dnppy/solar/solar.py) written for python 2 - adapted to python 3 (still some small errors sometimes) ### 2022-01-31 ### dnppy_solar.py ###
__author__ = "Jwely"
...
class solar:
    ...
    def get_true_solar(self):
        ...
        if self.equation_of_time is None:
            self.get_equation_of_time() ### Correction
...
if __name__ == "__main__":
    ...
    # further test
    mylat, mylon = numpy.mgrid[70:-71:-1,-180:181:1]
    my_datestamp = datetime.today()
    my_tz = 1
    sm2 = solar(mylat,mylon,my_datestamp,my_tz)
    sm2.get_azimuth()
    sm2.get_elevation()
    
    azimuth = sm2.get_azimuth()
    elevation = sm2.get_elevation()